...
def create_a_todo(todo: TodoCreate, session: Annotated[Session, Depends(get_dep)]):
    return create_todo(todo=todo,session=session)

@router.get("/{todo_id}", response_model=TodoResponse)
def get_a_todo(todo_id: int, session: Annotated[Session, Depends(get_dep)]):
...
def update_a_todo(todo_id: int,todo_data: TodoUpdate, session: Annotated[Session, Depends(get_dep)]):
    return update_todo(id=todo_id,todo_data=todo_data, session=session)

@router.delete("/{todo_id}")
def delete_a_todo(todo_id: int, session: Annotated[Session, Depends(get_dep)]):
    return delete_a_todo(id=todo_id, session=session)
