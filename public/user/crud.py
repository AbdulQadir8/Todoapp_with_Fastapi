...
def get_user(id: int, session: Annotated[Session, Depends(get_dep)]):
...
def get_users(offset: int=0,limit: int=20,session =  Annotated[Session, Depends(get_dep)]):
        statement = select(User).offset(offset).limit(limit)
        users = session.exec(statement).all()
        return users

def create_user(user: UserCreate, session: Annotated[Session, Depends(get_dep)]):    
...
def update_user(id: int,user_data: UserUpdate, session: Annotated[Session, Depends(get_dep)]):
        ...
        session.commit(user_to_update)
        session.refresh(user_to_update)
        return user_to_update


def delete_user(id: int, session: Annotated[Session, Depends(get_dep)]):
